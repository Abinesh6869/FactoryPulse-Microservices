...
import org.cts.fp_reporting.client.AnalyticsClient;
...
public class ReportService {
    ...
    public ReportResponse generateReport(ReportRequest request,
        ...
        // Build metrics by calling peer services
        try {
        ...
        try { identityClient.recordAuditLog(new AuditLogRequest("GENERATE_REPORT", "Report",
    ...
    private Map<String, Object> buildMetrics(Map<String, Object> parameters, String scope) {
        ...
        if ("LINE".equalsIgnoreCase(scope)) {
            ...
            // OEE — from fp_analytics
            try {
                var resp = analyticsClient.getOEEByLineAndDateRange(lineId, from.toString(), to.toString(), 10000, 0);
                if (resp != null && resp.getData() != null && resp.getData().getContent() != null) {
                    List<OEERecordResponse> list = resp.getData().getContent();
    ...
    private ReportResponse toReportResponse(Report r) {
        ...
        try {
            ...
            if (r.getMetricsJson() != null && !r.getMetricsJson().isBlank())
                metrics = objectMapper.readValue(r.getMetricsJson(), new TypeReference<>() {});
        } catch (Exception e) {
